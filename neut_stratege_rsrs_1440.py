...
if __name__ == '__main__':
    ...
    ma_period = 10
    period_k0_lst = [22]  # N1 计算线性回归的K值
    period_std0_lst = [250]  # N2 计算斜率标准化的周期
    period_k1_lst = [22]  # N1 计算线性回归的K值
    period_std1_lst = [250]  # N2 计算斜率标准化的周期
    N_l_lst = [0.9]  # 多头开仓阈值
    N_s_lst = [-1.1]  # 空头开仓阈值
    P_ATR_LST = [40]  # ATR周期
    ATR_n_lst = [1.1]  # ATR倍数
    ...
    state_lst = []
    group_day = group_day.assign(ma=lambda df: (talib.MA(df['close'].shift(1).values, ma_period - 1)))\
    ...
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr', 'ma', 'close_n']] \
        ...
        for period_k0 in period_k0_lst:
            if len(day_atr) <= min(ma_period, period_k0):
                continue
            ...
            for period_std0 in period_std0_lst:
                day_atr = day_atr.assign(K_ma0=lambda df: talib.MA(df['K0'].shift(1).values, period_std0)) \
                    .assign(rsrs0=lambda df: (df.K0.shift(1) - df.K_ma0) * df['R0'].shift(1) * df['K0'].shift(1) /
                talib.STDDEV(df.K0.shift(1).values, timeperiod=period_std0, nbdev=1))
                for period_k1 in period_k1_lst:
                    ...
                    for period_std1 in period_std1_lst:
                        group_ = day_atr.assign(K_ma1=lambda df: talib.MA(df['K1'].shift(1).values, period_std1)) \
                            .assign(rsrs1=lambda df: (df.K1.shift(1) - df.K_ma1) * df['R1'].shift(1) *
                                                     df['K1'].shift(1) / talib.STDDEV(df.K1.shift(1).values,
                                                                                      timeperiod=period_std1, nbdev=1))[
                            ['tickid', 'close_n', 'rsrs0', 'rsrs1', 'atr', 'ma']] \
                        ...
                        for ATR_n in ATR_n_lst:
                            for N_l in N_l_lst:
                                for N_s in N_s_lst:
                                    ...
                                    e_time = '2017-01-01 00:00：00'
                                    t0 = time.time()
                                    for idx, _row in group_.iterrows():
                                        if position == 0:

                                            if (_row.rsrs1 > N_l) & (_row.c_ma > 0) & (_row.ma_zd > 0) & (_row.date_time[:13] != e_time[:13]):
                                                cost = _row.close
                                                position = 1
                                            ...
                                            elif (_row.rsrs0 < N_s) & (_row.c_ma < 0) & (_row.ma_zd < 0) & (_row.date_time[:13] != e_time[:13]):
                                                cost = _row.close
                                                position = -1
                                                s_time = _row.date_time
                                                value_start = value
                                            ...
                                            elif (_row.close > high_price_pre) & (_row.date_time[:13] != e_time[:13]):
                                                cost = _row.close
                                                s_time = _row.date_time
                                                value_start = value
                                                position = 1
                                                ...
                                                hold_price.append(cost)
                                                high_price.append(cost)
                                            elif (_row.close < low_price_pre) & (_row.date_time[:13] != e_time[:13]):
                                                cost = _row.close
                                                position = -1
                                                s_time = _row.date_time
                                                value_start = value
                                            ...
                                            if (_row.rsrs0 < N_s) & (_row.c_ma < 0) & (_row.ma_zd < 0):
                                                e_time = _row.date_time
                                                e_tickid = _row.tickid
                                                if cont_btc > 0:
                                            ...
                                            if (_row.rsrs1 > N_l) & (_row.c_ma > 0) & (_row.ma_zd > 0):
                                                e_time = _row.date_time
                                                e_tickid = _row.tickid
                                                if cont_btc > 0:
                                                    p_price_btc = _row.close_b * (1 - fee)
                                    ...
                                    signal_state_all = pd.DataFrame(
                                        signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                             ...
                                                             'position'])
                                    # signal_state_all.to_csv('cl_trend/data/signal_312_' + str(n) + '.csv')
                                    for (s_date, e_date) in date_lst:
                                        ...
                                        net_df.to_csv('cl_trend/data/data/312_' + period + '.csv')
    ...
    res = pd.DataFrame(
        state_lst, columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                            'trade_times', 'ave_r', 'ave_hold_days', 'win_r_3', 'win_r_5', 'ave_max', 'win_r_3_p',
                            'win_r_5_p', 'ave_max_p', 'art_N', 'art_n', 'period_k0', 'period_std0', 'period_k1',
                            'period_std1', 'n_l', 'n_s', 'ma_period', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_RSRS_' + str(n) + '.csv')
