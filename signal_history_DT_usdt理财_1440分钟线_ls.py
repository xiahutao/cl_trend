...
if __name__ == '__main__':
    ...
    road_period_l_lst = [i for i in range(5, 6)]  # 通道周期
    road_period_s_lst = [i for i in range(6, 7)]
    K1_lst = [i / 100 for i in range(45, 50, 5)]  # 浮动倍数
    K2_lst = [i / 100 for i in range(70, 75, 5)]  # 浮动倍数
    N1_lst = [20]  # 均线周期
    win_stop_lst = [0.2]

    P_ATR_LST = [14, 18, 22, 26, 28, 30]  # ATR周期
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'road_period_s', 'K1', 'K2', 'N1', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
