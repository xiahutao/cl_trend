...
if __name__ == '__main__':
    ...
    N1_lst = [i / 100 for i in range(80, 105, 1)]  # 下阈值
    N2_lst = [i / 100 for i in range(102, 140, 1)]  # 上阈值

    N_lst = [i for i in range(2, 20, 2)]
    ...
    fee = 0.0036
    date_lst = [('2017-09-01', '2019-05-30'), ('2017-09-01', '2018-01-01'), ('2018-01-01', '2018-07-01'),
                ('2018-07-01', '2019-01-01'), ('2019-01-01', '2019-05-30')]

    # date_lst = [('2017-01-01', '2019-02-30')]
    state_lst = []
    for N in N_lst:
        df_btc_ = df_btc_period.assign(factor_b=lambda df: Alphas.alpha135(df, N))[
            ['tickid', 'close', 'factor_b', 'close_b_1']].rename(columns={'close': 'close_b'})
        df_eth_ = df_eth_period.assign(factor_e=lambda df: Alphas.alpha135(df, N))[
            ['tickid', 'close', 'factor_e', 'close_e_1']].rename(columns={'close': 'close_e'})
        group_ = df_btc_.merge(df_eth_, on=['tickid']).sort_values(['tickid']) \
            .assign(apha_ratio=lambda df: df.factor_b / df.factor_e) \
...
res = pd.DataFrame(state_lst,
               columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                        ...
                        'N2', 'N', 'back_stime', 'back_etime'])
res.to_csv('cl_trend/data/neu_strategy_factor135_' + str(n) + '.csv')
