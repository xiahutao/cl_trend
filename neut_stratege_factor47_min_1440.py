...
if __name__ == '__main__':
    ...
    N1_lst = [i / 100 for i in range(76, 77, 1)]  # 下阈值
    N2_lst = [i / 100 for i in range(116, 117, 1)]  # 上阈值
    N_lst = [i for i in range(27, 28)]
    loss_stop_list = [1]
    df_btc_period = pd.read_csv('data/btc_index_' + period + '.csv').loc[
             :, ['tickid', 'close', 'high', 'low', 'open']].assign(close_b_1=lambda df: df.close.shift(1))
    df_eth_period = pd.read_csv('data/ethindex_' + period + '.csv').loc[
    ...
    df_btc = pd.read_csv('data/btc_index_1m.csv').loc[
             :, ['tickid', 'open', 'close']] \
        .rename(columns={'open': 'open_b', 'close': 'close_b'}).assign(close_b_1=lambda df: df.close_b.shift(1))
    df_eth = pd.read_csv('data/ethindex_1m.csv').loc[
             :, ['tickid', 'open', 'close']] \
    ...
    state_lst = []
    for N in N_lst:
        df_btc_ = df_btc_period.assign(factor_b=lambda df: Alphas.alpha047(df, N))[
            ['tickid', 'factor_b']]
        df_eth_ = df_eth_period.assign(factor_e=lambda df: Alphas.alpha047(df, N))[
            ['tickid', 'factor_e']]
        group_ = df_btc_.merge(df_eth_, on=['tickid']).sort_values(['tickid']) \
            ...
            .assign(date_time=lambda df: df.tickid.apply(lambda x: str(datetime.datetime.fromtimestamp(x+28800))))\
            ...
            for N1 in N1_lst:
                for N2 in N2_lst:
                    ...
                    for idx, _row in group_.iterrows():
                        if position == 0:
                            if (_row.apha_ratio < N1) & (_row.date_time[:10] != e_time[:10]):
                                position = 1
                            ...
                            elif (_row.apha_ratio > N2) & (_row.date_time[:10] != e_time[:10]):
                                position = -1
                                s_time = _row.date_time
                                value_start = value
                                k_price_eth = _row.close_e * (1 - fee)
                    ...
                    signal_state_all = pd.DataFrame(
                        signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                             ...
                                             'position'])
                    signal_state_all.to_csv('cl_trend/data/signal_320_' + str(n) + '.csv')
                    for (s_date, e_date) in date_lst:
                        ...
                        net_df.to_csv('cl_trend/data/data/320_' + period + '.csv')
...
res.to_csv('cl_trend/data/neu_strategy_factor47_loss_' + str(n) + '.csv')
