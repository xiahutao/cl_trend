...
if __name__ == '__main__':
    ...
    period_k1_lst = [i for i in range(10, 20, 1)]  # N1 计算线性回归的K值
    period_std1_lst = [i for i in range(10, 30, 3)]  # N2 计算斜率标准化的周期
    N_l_lst = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1]  # 多头开仓阈值
    N_s_lst = [-1]  # 空头开仓阈值
    win_stop_lst = [10]
    status_days_lst = [0]
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_rsrs_btc_re_entry_' + str(n) + 'l.csv')
