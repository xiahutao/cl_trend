...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(5, 6)]  # 短周期
    N2_lst = [i for i in range(7, 8, 1)]  # 中周期
    N3_lst = [i for i in range(13, 14, 1)]  # 长周期
    N4_lst = [i for i in range(5, 6)]  # 唐奇安通道数
    P_ATR_LST = [30]  # ATR周期
    ATR_n_lst = [1000000]  # ATR倍数
    status_days_lst = [0]
    win_stop_lst = [10]
    ...
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr']] \
            ...
            .reset_index(drop=True)[['tickid', 'atr', 'close_1', 'high', 'low']]
        print(day_atr)
        for N1 in N1_lst:
            for N2 in N2_lst:
                for N3 in N3_lst:
                    if (N1 >= N2) | (N2 >= N3) | (len(group) <= N3):
                        continue
                    day_atr['ma1'] = talib.MA(day_atr['close_1'].values, N1)
                    day_atr['ma2'] = talib.MA(day_atr['close_1'].values, N2)
                    day_atr['ma3'] = talib.MA(day_atr['close_1'].values, N3)
                    for N4 in N4_lst:
                        group_ = day_atr.assign(hhv=lambda df: talib.MAX(df['high'].shift(1).values, N4)) \
                            .assign(llv=lambda df: talib.MIN(df['low'].shift(1).values, N4))[
                            ['tickid', 'ma1', 'ma2', 'atr', 'ma3', 'close_1', 'hhv', 'llv']] \
                        ...
                        for ATR_n in ATR_n_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                if position == 0:
                                    if (_row.ma1 > _row.ma2) & (_row.close_1 > _row.ma3) & (_row.close > _row.hhv) & (_row.date_time[:10] != e_time[:10]):
                                        cost = _row.close
                                        position = 1
                                    ...
                                    elif (_row.ma1 < _row.ma2) & (_row.close_1 < _row.ma3) & (_row.close < _row.llv) & (_row.date_time[:10] != e_time[:10]):
                                        cost = _row.close
                                        position = -1
                                        s_time = _row.date_time
                                        value_start = value
                            ...
                            signal_state_all = pd.DataFrame(
                                signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                     ...
                                                     'position'])
                            signal_state_all.to_csv('cl_trend/data/signal_318_' + str(n) + '.csv')
                            for (s_date, e_date) in date_lst:
                                ...
                                # net_df.to_csv('cl_trend/data/data/318_' + period + '.csv')
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'N4', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_tld_' + str(n) + '.csv')