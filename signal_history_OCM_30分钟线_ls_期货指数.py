...
if __name__ == '__main__':
    ...
    # symble = 'yeeeth'
    x = 3
    symble_lst = ['ethusdt', 'btcusdt']
    n = 30  # 回测周期
    period = '30m'

    N1_lst = [8, 9, 10]  # 短周期周期1
    N2_lst = [95, 100, 105]  # 长周期周期2
    K1_lst = [s / 100 for s in range(5, 66, 5)]  # 近期比例
    K2_lst = [s / 100 for s in range(5, 66, 5)]  # 长期比例
    P_ATR_LST = [20]  # ATR周期
    ATR_n_lst = [10000, 0.5]  # ATR倍数
    ...
    for symble in symble_lst:
        if symble == 'ethusdt':
            ...
            group = group[(group['date_time'] >= s_date) & (group['date_time'] <= e_date)]
            group_day = pd.read_csv('data/btc_index_' + '1d' + '.csv').assign(date_time=lambda df: df.date_time + ' 00:00:00')
            group_day = group_day[(group_day['date_time'] >= s_date) & (group_day['date_time'] <= e_date)]
            # print(group)
            # print(group_day)
                                                ...
                                                if pos == 1:
                                                    ...
                                                    if pos > 1:
                                                        if _row.low < max(hold_price) - _row.atr * ATR_n:
                                                            ...
                                                            ret = (pos * s_price + (1 - pos) * cost) / s_price - 1
                                                            signal_row = []
                                                            ...
                                                            trad_times += 1
                                                            s_price = ((0.5 + 0.5 * K1) * (
                                                                    _row.HH_s - _row.LL_s) + _row.LL_s) * (1 - fee)
                                                            if _row.open > (0.5 + 0.5 * K1) * (
                                                                    _row.HH_s - _row.LL_s) + _row.LL_s:
                                                                s_price = _row.open * (1 - fee)
                                                        ...
                                                        if _row.high > min(hold_price) + _row.atr * ATR_n:
                                                            ...
                                                            e_time = _row.date_time
                                                            trad_times += 1
                                                            b_price = ((0.5 - 0.5 * K1) * (_row.HH_s - _row.LL_s) + _row.LL_s) * (1 + fee)
                                                            if _row.open < (0.5 - 0.5 * K1) * (_row.HH_s - _row.LL_s) + _row.LL_s:
                                                                b_price = _row.open * (1 + fee)
                                                            net1 = (pos * b_price + (1 - pos) * _row.close_1) / b_price
                                                            ret = (pos * b_price + (1 - pos) * cost) / b_price - 1
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_ocmup_' + str(x) + str(n) + 'ls17atr_usdt.csv')
