...
#    data=pd.read_csv('data/position_l.csv',index_col=0)
    ...
    for N1 in N1_lst:
        for N2 in N2_lst:
            for N3 in N3_lst:
                ...
                e_lst=list()
                date_lst=list()
                method='neut_stratege_aapv'+'_'+str(N1)+'_'+str(N2)+'_'+str(N3)
                print(method)
    ...
    #            ethbtc_price.columns=['date','eth_btc']
                ...
                ethbtc_price=ethbtc_price.assign(pivot1=lambda df:(df.open+df.low+df.close)/3)\
                                         .assign(pivot2=lambda df:(df.open+df.high+df.close)/3)\
                                         .assign(resis=lambda df:(2*df.pivot1-df.low))\
                                         .assign(support=lambda df:(2*df.pivot2-df.high))\
                                         .assign(stat_sign_B=lambda df:[aapv(_row) for idx,_row in df.iterrows()])\
                                         .assign(stat_sign_S=lambda df:[aapv2(_row) for idx,_row in df.iterrows()])\
                                         .assign(signB=lambda df:tb.MA(df['stat_sign_B'].values,N1))\
                                         .assign(signS=lambda df:tb.MA(df['stat_sign_S'].values,N1))\
                                         .assign(signB_1=lambda df:df.signB.shift(1))\
                                         .assign(signS_1=lambda df:df.signS.shift(1))\
#                                         .assign(signB_1=lambda df:df.signB.shift(1))\
#                                         .assign(signS_1=lambda df:df.signS.shift(1))
            ...
            #        print(num%5)
             ...
             #       if (position==0) and (row_['GarmanKlassYang_Vol_1']>0.0) :
                    if (position==0):
                        ...
                        elif (row_['signS_1']>N2):
                            position=2
                            trade_nums=trade_nums+1
                    else:    
                        if (position==1) & (row_['signS_1']>N3):
            #                print('************')
                            position=0
                        elif (position==2) & (row_['signB_1']>N3):
                            position=0
...
#    res.to_csv('result/neu_strategy_aapv.csv')      
