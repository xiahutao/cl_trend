...
if __name__ == '__main__':
    ...
    for N1 in N1_lst:
        for N2 in N2_lst:
            ...
            e_lst = list()
            date_lst = list()
            method = 'neut_stratege_tcs' + '_' + str(N1) + '_' + str(N2)
            print(method)
            ...
            ethbtc_price = ethbtc_price.assign(mid=lambda df: tb.MA(df['close'].values, N1)) \
                .assign(delta=lambda df: df.close - df.close.shift(1)) \
                .assign(delta_abs=lambda df: df.delta.apply(lambda s: abs(s))) \
                .assign(delta_abs_ma=lambda df: tb.MA(df['delta_abs'].values, N1)) \
                .assign(delta_abs_ma_pb=lambda df: df.delta_abs_ma * N2) \
                .assign(up=lambda df: df.delta_abs_ma_pb + df.mid) \
                .assign(up_1=lambda df: df.up.shift(1)) \
                .assign(down=lambda df: df.mid - df.delta_abs_ma_pb) \
                .assign(down_1=lambda df: df.down.shift(1)) \
                .assign(close_1=lambda df: df.close.shift(1)) \
                .assign(mid_1=lambda df: df.mid.shift(1))
            ...
            for idx, row_ in data.iterrows():
                ...
                if (position == 0):
                    if (row_['close_1'] > row_['up_1']):
                        position = 1
                        trade_nums = trade_nums + 1
                    elif (row_['close_1'] < row_['down_1']):
                        position = 2
                        trade_nums = trade_nums + 1
                else:
                    if (position == 1) & (row_['close_1'] < row_['mid_1']):
                        #                print('************')
                        position = 0
                    elif (position == 2) & (row_['close_1'] > row_['mid_1']):
                        position = 0
