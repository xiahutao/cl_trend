...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(8, 9, 3)]  # 统计周期
    k_lst = [i/10 for i in range(16, 17, 3)]  # 阈值
                ...
                for k in k_lst:
                    ...
                    for (s_date, e_date) in date_lst:
                        group_ = group__[(group__['date_time'] >= s_date) & (group__['date_time'] <= e_date)].dropna() \
                            .reset_index(drop=True)
                        # group_.to_csv('cl_trend/data/tcs_signal_' + symble + '_' + period + '.csv')
                        ...
                        for ATR_n in ATR_n_lst:
                            for win_stop in win_stop_lst:
                                for status_day in status_days_lst:
                                    print(symble)
                                    if (symble == 'ethindex') | (symble == 'eosbtc') | (symble == 'ethbtc'):
                                        signal_lst = []
                                        trad_times = 0
                                        net = 1
                                        net_lst = []
                                        ...
                                        net_df.to_csv(
                                            'cl_trend/data/data/future_017_' + period + '_ethusd.csv')
                                        ...
                                        # signal_state.to_csv('cl_trend/data/signal_tcs_' + str(n) + '_' + symble + '_.csv')
                                        ...
                                        net_df.to_csv(
                                            'cl_trend/data/data/future_017_' + period + '_xbtusd.csv')
                                        # net_df[['date_time', 'net', 'close']].plot(
                                        #     x='date_time', kind='line', grid=True,
                                        #     title=symble + '_' + period)
                                        #
                                        ...
                                        signal_state = pd.DataFrame(
                                            signal_lst, columns=['s_time', 'e_time', 'k_price', 'p_price',
                                                                 ...
                                                                 'bspk'])
                                        signal_state.to_csv(
                                            'cl_trend/data/signal_tcs_' + str(n) + '_' + symble + '_.csv')
                                        # df_lst.append(signal_state)
                                        win_r, odds, ave_r, mid_r = get_winR_odds(signal_state.ret.tolist())
