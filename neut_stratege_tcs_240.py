...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(25, 26, 5)]  # 统计周期
    k_lst = [i / 10 for i in range(65, 66, 5)]  # 阈值
    P_ATR_LST = [30]  # ATR周期
                    ...
                    for win_stop in win_stop_lst:
                        for status_day in status_days_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                if position == 0:
                                    if (_row.close > _row.up) & (_row.date_time[:16] != e_time[:16]):
                                        cost = _row.close
                                        position = 1
                                    ...
                                    elif (_row.close < _row.down) & (_row.date_time[:16] != e_time[:16]):
                                        cost = _row.close
                                        position = -1
                                        s_time = _row.date_time
                                        value_start = value
