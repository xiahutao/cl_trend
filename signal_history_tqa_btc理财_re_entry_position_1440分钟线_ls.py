...
if __name__ == '__main__':
    ...
    # symble = 'yeeeth'
    x = 8
    symble_lst = ['ethindex', 'btc_index']
    n = 60  # 回测周期
    period = '60m'

    road_period_l_lst = [22]  # 唐奇安通道周期
    road_period_s_lst = [16]  # 唐奇安通道周期
    ma_period_lst = [20]
    P_ATR_LST = [16]  # ATR周期
    ATR_n_lst = [0.6]  # ATR倍数
    # status_days_lst = [0]
                    ...
                    for road_period_s in road_period_s_lst:
                        ...
                        for (s_date, e_date) in date_lst:
                            group_ = group__[
                                ...
                                .reset_index(drop=True)
                            # group_.to_csv('cl_trend/data/tqa_signal_' + symble + '_' + period + '.csv')
                            if len(group_)<20:
                                continue
                                ...
                                for win_stop in winstop_lst:
                                    for status_day in status_days_lst:
                                        ...
                                        if (symble == 'ethindex') | (symble == 'eosbtc'):
                                                ...
                                                if pos == 0:
                                                    ...
                                                    if pos > 0:
                                                        ...
                                                        if (_row.low < _row.l10) & (_row.c_ma < 0) & (_row.ma_zd < 0):
                                                            ...
                                                            net = net1 * net
                                                            status = 0
                                                        elif _row.high >= cost / (1 + fee) * (1 + win_stop):

                                                            trad_times += 1
                                                            high_price.append(_row.high)
                                                            e_time = _row.date_time
                                                            s_price = cost / (1 + fee) * (1 + win_stop) * (1 - fee)
                                                            if min(_row.open, _row.high) >= cost / (1 + fee) * (1 + win_stop):
                                                                s_price = min(_row.open, _row.high) * (1 - fee)
                                                        ...
                                                        if (_row.high > _row.h10) & (_row.c_ma > 0) & (_row.ma_zd > 0):
                                                            ...
                                                            status = 0
                                                        elif _row.low <= cost / (1-fee) * (1-win_stop):
                                                            trad_times += 1
                                                            e_time = _row.date_time
                                                            b_price = cost / (1-fee) * (1-win_stop) * (1 + fee)
                                                            if _row.open < cost / (1-fee) * (1-win_stop):
                                                                b_price = _row.open * (1 + fee)
                                            ...
                                            #
                                            # net_df.to_csv('cl_trend/data/data/future_001_' + period + '_ethusd.csv')
                                                ...
                                                if pos == 1:
                                                    ...
                                                    if pos > 1:
                                                        if (_row.low < _row.l10) & (_row.c_ma < 0) & (_row.ma_zd < 0):
                                                            ...
                                                            high_price.append(_row.high)
                                                            e_time = _row.date_time
                                                            s_price = cost / (1+fee) * (1 + win_stop) * (
                                                                    1 - fee)
                                                            if min(_row.open, _row.high) >= cost / (1+fee) * (
                                                                    1 + win_stop):
                                            ...
                                            #                        'date_time': group_.date_time.tolist()})
                                            #
                                            # net_df.to_csv('cl_trend/data/data/future_001_' + period + '_xbtusd.csv')
                                            # net_df[['date_time', 'net', 'close']].plot(
                                            #     x='date_time', kind='line', grid=True,
                                            #     title=symble + '_' + period)
                                            #
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'road_period_s', 'ma_period', 'status_day', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_tqa_position_entry' + str(n) + 'ls.csv')
