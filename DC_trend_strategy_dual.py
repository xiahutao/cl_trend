            ...
            for road_period in road_period_lst:
                for N_ATR in P_ATR_LST:
                    for K in K_lst:
                        ...
                        print(N_ATR)
                        method = 'tqa' + str(N1) + '_' + str(N_ATR) + '_' + str(ATR_n) + '_' + str(road_period)+'_'+str(K)
                        signal_lst = []
                        trad_times = 0
                        ...
                        if len(group) > N1:
                            net = 1
                            net_lst = []
                            group=group.assign(HH=lambda df:tb.MAX(df.high.values,road_period)) \
                                        ...
                                        .assign(HHLC=lambda df:df.HH-df.LC) \
                                        .assign(HCLL=lambda df:df.HC-df.LL)\
                                        .assign(max_HCL=lambda df:[ max(row['HHLC'],row['HCLL']) for idx,row in df.iterrows()])\
                                        .assign(BuyRange=lambda df: K*df.max_HCL+df.open) \
                                        .assign(ma=lambda df:tb.MA(df.close.values,N1)) \
                            ...
                            group = group.assign(close_1=lambda df: df.close.shift(1)) \
                                        ...
                                        .assign(BuyRange_1=lambda df: df.BuyRange.shift(1)) \
                            ...
                            for idx, _row in group.iterrows():
                                    ...
                                    if (position == 0) & (_row.high > _row.BuyRange_1) & (_row.c_ma > 0) & (_row.ma_zd > 0):
                                    position = 1
                                    s_time = _row.date_time
                                    cost = _row.BuyRange_1
                                    hold_price = []
                                    high_price = []
                                    hold_price.append(_row.BuyRange_1)
