...
if __name__ == '__main__':
    ...
    N2_lst = [i for i in range(4, 5)]  # 统计周期
    N3_lst = [i for i in range(3, 4)]  # 计数上限

    N4_lst = [i for i in range(2, 3)]  # 计数阈值
    N5_lst = [i for i in range(3, 4)]  # 统计周期
    N6_lst = [i for i in range(6, 7)]  # 计数上限
    win_stop_lst = [10, 0.3, 0.25, 0.2, 0.15, 0.1]
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'N2', 'N3', 'N1s', 'N2s', 'N3s', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_gzw_usdt' + str(n) + 'ls.csv')
