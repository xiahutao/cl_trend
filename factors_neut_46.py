...
def price_volume(df, corr_window):
    # 计算因子：量价相关系数
    factor = tb.CORREL(df['close'], df['volume'], corr_window)
    return factor
...
if __name__ == '__main__':
    ...
    N1_lst = np.arange(0.8, 1, 0.01)  # 下阈值
    N2_lst = np.arange(1.01, 1.11, 0.01)  # 上阈值
    #
    #    N1_lst=[0.99] # 下阈值
    #    N2_lst=[1.01]# 上阈值
    ...
    for symbol in symbols:
        try:
            if num == 0:
                dataf = pd.read_csv('data/' + symbol + '_' + period + '_' + exchange + '_' + sday + '_' + eday + '.csv',
                                    index_col=0)
                print(symbol)
                Alpha = Alphas(dataf)
                ...
                df_m = df_m[['date']]
                df_m['kind'] = symbol
                df_m[col_name] = Alphas.alpha046(dataf)
                res = df_m
            else:
                dataf = pd.read_csv('data/' + symbol + '_' + period + '_' + exchange + '_' + sday + '_' + eday + '.csv',
                                    index_col=0)
    ...
    for sy in symbols:
        ...
        #        print(data_file)
        if num == 0:
            price = pd.read_csv('data/' + sy + '_' + period + '_' + exchange + '_' + sday + '_' + eday + '.csv',
                                index_col=0)
            price = price[['date', 'close']]
            price['kind'] = sy
        else:
            temp = pd.read_csv('data/' + sy + '_' + period + '_' + exchange + '_' + sday + '_' + eday + '.csv',
                               index_col=0)
    ...
    res.to_csv('result/neu_strategy_aph46.csv')
