...
if __name__ == '__main__':
    ...
    K1_lst = [s / 100 for s in range(25, 30, 5)]  # 近期比例
    K2_lst = [s / 100 for s in range(5, 10, 5)]  # 长期比例
        ...
        for symble in symble_lst:
            if symble == 'ethusdt':
                ...
                group = pd.read_csv('data/btcusdt_' + period + '.csv').rename(columns={'date': 'date_time'})\
                    .assign(date_time=lambda df: df.date_time.apply(lambda x: str(x)))
                group = group[(group['date_time'] >= s_date) & (group['date_time'] <= e_date)]
                group_day = pd.read_csv('data/btcusdt_' + '1440m' + '.csv').assign(
                    date_time=lambda df: df.date_time.apply(lambda x: str(x)))
                group_day = group_day[(group_day['date_time'] >= s_date) & (group_day['date_time'] <= e_date)]
                # print(group)
                # print(group_day)
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_ocm_' + str(n) + 's' + str(x) + '.csv')
