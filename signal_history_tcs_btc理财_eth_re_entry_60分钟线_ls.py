...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(10, 101, 5)]  # 统计周期
    k_lst = [i/10 for i in range(20, 101, 5)]  # 阈值
                                    ...
                                    if (symble == 'ethusd') | (symble == 'eosbtc') | (symble == 'ethbtc'):
                                        ...
                                        for idx, _row in group_.iterrows():
                                            if pos == 0:
                                                ...
                                                if pos == 1:
                                                    if _row.low < max(_row.ma, (max(hold_price) - _row.atr * ATR_n)):

                                                        trad_times += 1
                                                        high_price.append(_row.high)
                                                        e_time = _row.date_time
                                                        s_price = max(_row.ma, (max(hold_price) - _row.atr * ATR_n)) * (1 - fee)
                                                        if min(_row.open, _row.high) < max(_row.ma, (max(hold_price) - _row.atr * ATR_n)):
                                                    ...
                                                    if _row.high > min(_row.ma, (min(hold_price) + _row.atr * ATR_n)):
                                                        trad_times += 1
                                                        e_time = _row.date_time
                                                        b_price = min(_row.ma, (min(hold_price) + _row.atr * ATR_n)) * (
                                                                1 + fee)
                                                        if max(_row.open, _row.low) > min(_row.ma, (min(hold_price) + _row.atr * ATR_n)):
                                            ...
                                            if pos == 1:
                                                ...
                                                if pos > 1:
                                                    ...
                                                    if _row.low < max(_row.ma, (max(hold_price) - _row.atr * ATR_n)):
                                                        ...
                                                        high_price.append(_row.high)
                                                        e_time = _row.date_time
                                                        s_price = max(_row.ma, (max(hold_price) - _row.atr * ATR_n)) * (
                                                                1 - fee)
                                                        if min(_row.open, _row.high) < max(_row.ma, (max(hold_price) - _row.atr * ATR_n)):
                                                            s_price = min(_row.open, _row.high) * (1 - fee)
                                                        net1 = (pos * s_price + (
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_tcs_eth_' + str(n) + 'ls.csv')
