...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(4, 5)]  # 短周期
    N2_lst = [i for i in range(12, 13, 1)]  # 中周期
    N3_lst = [i for i in range(35, 36, 1)]  # 长周期
    P_ATR_LST = [36]  # ATR周期
    ATR_n_lst = [0.5]  # ATR倍数
    # status_days_lst = [0]
    status_days_lst = [i for i in range(21)]
    win_stop_lst = [10]

    lever = 1  # 杠杆率，btc初始仓位pos=1,usdt初始仓位pos=0;做多情况下，btc仓位1+lever,usdt仓位-lever;
    # 做空情况下，btc仓位1-lever,usdt仓位lever
    fee = 0.002
    date_lst = [('2017-01-01', '2019-02-30'), ('2017-01-01', '2018-01-01'), ('2018-01-01', '2018-07-01'),
                                                ...
                                                for idx, _row in group_.iterrows():
                                                    ...
                                                    if pos == 0:
                                                        ...
                                                        if pos == 1:
                                                            ...
                                                            if _row.high >= (_row.ma2*N1*(N2-1)-_row.ma1*N2*(N1-1))/(N2-N1):
                                                                b_price = (_row.ma2*N1*(N2-1)-_row.ma1*N2*(N1-1))/(N2-N1) * (1 + fee)
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'road_period_s', 'ma_period', 'win_stop', 'status_day', 's_time', 'e_time'])
    print(signal_state)
