...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(5, 6, 1)]  # 统计周期
    k_lst = [i / 10 for i in range(23, 24, 1)]  # 阈值
    P_ATR_LST = [30]  # ATR周期
    ...
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr']] \
            ...
            .reset_index(drop=True)[['tickid', 'atr', 'close_1', 'delta']]
        print(day_atr)
        for ma in ma_lst:
            for k in k_lst:
                group_ = day_atr.assign(ma=lambda df: talib.MA(df['close_1'].values, ma)) \
                    ...
                    .assign(down=lambda df: df.ma - k * df.delta_ma)[['tickid', 'up', 'down', 'atr', 'ma']] \
                    ...
                    for win_stop in win_stop_lst:
                        for status_day in status_days_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                if position == 0:
                                    if (_row.close > _row.up) & (_row.date_time[:10] != e_time[:10]):
                                        cost = _row.close
                                        position = 1
                                    ...
                                    elif (_row.close < _row.down) & (_row.date_time[:10] != e_time[:10]):
                                        cost = _row.close
                                        position = -1
                                        s_time = _row.date_time
                                        value_start = value
                            ...
                            signal_state_all = pd.DataFrame(
                                signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                     ...
                                                     'position'])
                            # signal_state_all.to_csv('cl_trend/data/signal_317_' + str(n) + '.csv')
                            for (s_date, e_date) in date_lst:
                                ...
                                net_df.to_csv('cl_trend/data/data/317_' + period + '.csv')
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'win_r_3_p', 'win_r_5_p', 'ave_max_p', 'art_N', 'art_n', 'ma', 'k', 'win_stop',
                                'status_day', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_tcs_' + str(n) + '.csv')