...
if __name__ == '__main__':
    ...
    road_period_lst = [i for i in range(4, 5)]  # 唐奇安通道周期
    k_lst = [i/100 for i in range(70, 71, 5)]
    ma_period_lst = [10]
    P_ATR_LST = [24]  # ATR周期
    ATR_n_lst = [0.6]  # ATR倍数
    status_days_lst = [0]
    ...
    for N_ATR in P_ATR_LST:
        ...
        for ma_period in ma_period_lst:
            ...
            day_atr = group_day \
                .assign(close_n=lambda df: df['close'].shift(ma_period))[['tickid', 'atr', 'ma', 'close_n']] \
                .assign(atr=lambda df: df.atr.shift(1)) \
                .merge(group, on=['tickid'], how='right') \
                .sort_values(['tickid']).fillna(method='ffill') \
                .reset_index(drop=True)[['tickid', 'atr', 'ma', 'close_n', 'high', 'low', 'open', 'close']]
            print(day_atr)
            for road_period in road_period_lst:

                if len(day_atr) < road_period:
                    continue
                day_atr = day_atr.assign(
                    HH_LC=lambda df: tb.MAX(df.high.values, road_period) - tb.MIN(df.close.values, road_period)).assign(
                    HC_LL=lambda df: tb.MAX(df.close.values, road_period) - tb.MIN(df.low.values, road_period)).assign(
                    max_HCL=lambda df: [max(row.HH_LC, row.HC_LL) for idx, row in df.iterrows()])
                for k in k_lst:
                    group_ = day_atr.assign(buyRange_1=lambda df: k * df.max_HCL.shift(1) + df.open) \
                        .assign(sellRange_1=lambda df: df.open - k * df.max_HCL.shift(1))[
                        ['tickid', 'close_n', 'buyRange_1', 'sellRange_1', 'atr', 'ma']] \
                            ...
                            for status_day in status_days_lst:
                                ...
                                for idx, _row in group_.iterrows():
                                    if position == 0:
                                        if (status >= 0) & (status < status_day):
                                            ...
                                            if (_row.close > _row.buyRange_1) & (_row.c_ma > 0) & (_row.ma_zd > 0):
                                                cost = _row.close
                                                position = 1
                                                s_time = _row.date_time
                                                value_start = value
                                                ...
                                                hold_price.append(cost)
                                                high_price.append(cost)
                                            elif (_row.close < _row.sellRange_1) & (_row.c_ma < 0) & (_row.ma_zd < 0):
                                                cost = _row.close
                                                position = -1
                                                s_time = _row.date_time
                                                value_start = value
                                        ...
                                        if (_row.close < _row.sellRange_1) & (_row.c_ma < 0) & (_row.ma_zd < 0):
                                ...
                                signal_state_all = pd.DataFrame(
                                    signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                         ...
                                                         'position'])
                                # signal_state_all.to_csv('cl_trend/data/signal_310_' + str(n) + '.csv')
                                for (s_date, e_date) in date_lst:
                                    ...
                                    net_df.to_csv('cl_trend/data/data/310_' + period + '.csv')
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'win_r_3_p', 'win_r_5_p', 'ave_max_p', 'art_N', 'art_n', 'road_period', 'k',
                                'ma_period', 'win_stop', 'status_day', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_DT_' + str(n) + '.csv')
