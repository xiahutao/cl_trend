...
if __name__ == '__main__':
    ...
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr']] \
            ...
            .reset_index(drop=True)[['tickid', 'atr', 'close_1', 'high', 'low', 'open', 'close']]
        print(day_atr)
        for N2 in N2_lst:
            for N5 in N5_lst:
                ...
                for N3 in N3_lst:
                    for N6 in N6_lst:
                        ...
                        group_ = day_atr \
                            ...
                            ['tickid', 'hhv_num_chg', 'llv_num_chg', 'atr']] \
                            .merge(hq_data, on=['tickid'], how='right').sort_values(['tickid']).fillna(method='ffill') \
                            .merge(df_org, on='tickid').sort_values(['tickid']) \
                            .assign(
                            date_time=lambda df: df.tickid.apply(lambda x: str(datetime.datetime.fromtimestamp(x)))) \
                                    ...
                                    for win_stop in win_stop_lst:
                                        for status_day in status_days_lst:
                                                ...
                                                if position == 0:
                                                    if (status >= 0) & (status < status_day * 1440):
                                                        ...
                                                        if _row.close > high_price_pre:
                                                            ...
                                                            value_b = value_b + cont_btc * (
                                                                        1 / k_price_btc - 1 / _row.close_b)
                                                    ...
                                                    if _row.llv_num_chg > N4:
                                                        ...
                                                        if cont_btc > 0:
                                                            ...
                                                            p_price_eth = _row.close_e * (1 - fee)
                                                        value_b = value_b + cont_btc * (
                                                                    1 / _row.close_b_1 - 1 / p_price_btc)
                                                        value_e = value_e + cont_eth * (
                                                                    p_price_eth - _row.close_e_1) / 1000000
                                                        value = value_b + value_e
                                                        value_end = value
                                                        ...
                                                        hold_price.append(cost)
                                                        low_price.append(cost)
                                                    elif (_row.hhv_num_chg <= N1) & (
                                                            _row.low < max(hold_price) - _row.atr * ATR_n):
                                                        e_time = _row.date_time
                                                        if cont_btc > 0:
                                                            p_price_btc = _row.close_b * (1 - fee)
                                                            p_price_eth = _row.close_e * (1 + fee)
                                                        ...
                                                        value_b = value_b + cont_btc * (
                                                                    1 / _row.close_b_1 - 1 / _row.close_b)
                                                        value_e = value_e + cont_eth * (
                                                                    _row.close_e - _row.close_e_1) / 1000000
                                                        value = value_b + value_e
                                                    ...
                                                    if _row.hhv_num_chg > N1:
                                                        ...
                                                        high_price.append(cost)
                                                            ...
                                                            _row.high > min(hold_price) + _row.atr * ATR_n):
                                                        e_time = _row.date_time
                                                        if cont_btc > 0:
                                                            p_price_btc = _row.close_b * (1 - fee)
                                                            p_price_eth = _row.close_e * (1 + fee)
                                            ...
                                            signal_state_all = pd.DataFrame(
                                                signal_lst,
                                                columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                            ...
                                            for (s_date, e_date) in date_lst:
                                                value_df_ = value_df[(value_df['date_time'] >= s_date) & (
                                                            value_df['date_time'] <= e_date)] \
                                                ...
                                                win_R_3_p, win_R_5_p, ave_max_p = get_winR_max(
                                                    signal_state.max_ret_price.tolist())
                                                total_ret = net_lst[-1]
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'win_r_3_p', 'win_r_5_p', 'ave_max_p', 'art_N', 'art_n', 'N1', 'N2', 'N3', 'N1s', 'N2s',
                                'N3s', 'win_stop', 'status_day', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_gzw_' + str(n) + '.csv')
