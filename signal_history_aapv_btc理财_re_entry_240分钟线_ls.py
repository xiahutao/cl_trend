...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(11, 12, 1)]  # 统计周期
    k_lst = [i/100 for i in range(70, 71, 5)]  # 阈值
    p_lst = [i/100 for i in range(65, 66, 5)]  # 阈值
    win_stop_lst = [10, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15]
    P_ATR_LST = [16]  # ATR周期
    ATR_n_lst = [1.3]  # ATR倍数
    status_days_lst = [0]
    ...
    for symble in symble_lst:
        ...
        group = group.assign(pivot1=lambda df: (df.open + df.low + df.close) / 3) \
            ...
            .assign(resis_s=lambda df: (df.close - (2 * df.pivot2 - df.high)))\
            .assign(stat_sign_l=lambda df: df.resis_l.apply(lambda x: transfer_resis_stat(x)))\
            .assign(stat_sign_s=lambda df: df.resis_s.apply(lambda x: transfer_resis_stat(x)))
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_aapv_' + str(n) + 'ls1.csv')
