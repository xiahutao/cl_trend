...
if __name__ == '__main__':
    ...
    n = 240  # 回测周期
    period = '240m'
    road_period_lst = [i for i in range(3, 10)]  # 通道周期
    K2_lst = [i/100 for i in range(40, 91, 5)]  # 浮动倍数
    N1_lst = [20, 15, 10]  # 均线周期
            ...
            for N1 in N1_lst:
                ...
                for road_period in road_period_lst:
                    if len(day_atr) <= road_period:
                        continue
                    day_atr = day_atr.assign(HH=lambda df: tb.MAX(df.high.values, road_period)) \
                    ...
                    for K2 in K2_lst:
                        group__ = day_atr.assign(sellRange_1=lambda df: df.open - K2 * df.max_HCL.shift(1))
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'K1', 'N1', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_dt_btcusdt_' + str(n) + 's.csv')
