...
if __name__ == '__main__':
    ...
    P_ATR_LST = [20]  # ATR周期
    band_lst = [0.05]  # 网格初始宽度
    ATR_n_lst = [1]  # ATR倍数
    weight_lst = [0.02]  # 每次补仓权重
    ...
    for symble in symble_lst:
        ...
        group_day['top'], group_day['mid'], group_day['bottom'] = talib.BBANDS(
            group_day['close'].values, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
        day_atr = group_day[['date_time', 'bottom']] \
                ...
                if len(group) > 20:
                    ...
                    for idx, _row in day_atr.iterrows():
                        ...
                        if (position < weight) & (_row.low < _row.bottom):
                            ...
                            net = net * (0.7 + 0.3 * _row.close / _row.bottom * (1 - fee))
                            net_lst.append(net)
                            position_lst.append(position)
                            signal_row = []
                        ...
                        elif (position >= weight) & (position <= 1 - weight) & (_row.low <= base_price * (1 - band_low)):
                            trad_times += 1
                            position = position + weight
                            s_time = _row.date_time
                    ...
                    print('参数=', band, weight)

    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                                    'trade_times', 'position', 'band', 'weight'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_wangge_all' + '_' + str(x) + '.csv')
