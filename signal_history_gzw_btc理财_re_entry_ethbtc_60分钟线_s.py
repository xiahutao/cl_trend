...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(2, 4)]  # 计数阈值
    N2_lst = [i for i in range(25, 26, 1)]  # 统计周期
    N3_lst = [i for i in range(12, 13, 1)]  # 计数上限

    N4_lst = [i for i in range(4, 8)]  # 计数阈值
    N5_lst = [i for i in range(4, 20, 2)]  # 统计周期
    N6_lst = [i for i in range(10, 30, 2)]  # 计数上限
    win_stop_lst = [10]
    status_days_lst = [0]
    P_ATR_LST = [30]  # ATR周期
    ...
    for symble in symble_lst:
        ...
        if symble == 'btcindex':
            ...
            group = pd.read_csv('data/ethbtc_' + period + '.csv').rename(columns={'date': 'date_time'}) \
                .assign(date_time=lambda df: df.date_time.apply(lambda x: str(x)))\
                .assign(close_1=lambda df: df.close.shift(1))
            group_day = pd.read_csv('data/ethbtc_' + '1440m' + '.csv')
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_gzw_usdt_re_entry' + str(n) + 's.csv')
