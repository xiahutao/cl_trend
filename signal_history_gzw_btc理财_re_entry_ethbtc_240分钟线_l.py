...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(2, 6)]  # 计数阈值
    N2_lst = [i for i in range(3, 8, 1)]  # 统计周期
    N3_lst = [i for i in range(5, 15, 1)]  # 计数上限

    N4_lst = [i for i in range(2, 4)]  # 计数阈值
    N5_lst = [i for i in range(2, 6)]  # 统计周期
    N6_lst = [i for i in range(3, 6)]  # 计数上限
    win_stop_lst = [10]
    status_days_lst = [0]
    P_ATR_LST = [30]  # ATR周期
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_gzw_usdt_re_entry' + str(n) + 'l.csv')
