...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(85, 86)]  # 统计周期
    k_lst = [i / 100 for i in range(45, 46, 5)]  # 阈值
    p_lst = [i / 100 for i in range(40, 41, 5)]  # 阈值

    P_ATR_LST = [20, 24, 28, 32, 36, 40, 12, 16]  # ATR周期
    ATR_n_lst = [100000, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4]  # ATR倍数
    status_days_lst = [0]
    win_stop_lst = [10]
    ...
    for N_ATR in P_ATR_LST:
        ...
        for ma in ma_lst:
            ...
            group_[group_['tickid']<1510000000].to_csv('cl_trend/data/group.csv')
            for k in k_lst:
                for p in p_lst:
                    if k <= p:
                        continue
                        ...
                        for idx, _row in group_.iterrows():
                            if position == 0:
                                ...
                                if _row.sign_s_1 > k:
                                    ...
                                    signal_lst.append(signal_row)
                                    position = 0
                                elif (_row.sign_l_1 <= k) & (_row.close < max(hold_price) - _row.atr * ATR_n):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                ...
                                if _row.sign_l_1 > k:
                                    ...
                                    hold_price.append(cost)
                                    high_price.append(cost)
                                elif (_row.sign_l_1 > p) & (_row.sign_s_1 <= k):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                ...
                                elif ((_row.sign_s_1 <= k)) & (_row.close > min(hold_price) + _row.atr * ATR_n):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
