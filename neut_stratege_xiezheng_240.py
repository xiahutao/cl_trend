...
if __name__ == '__main__':
    ...
    period_std_lst = [i for i in range(25, 50, 1)]  # 标准化窗口
    period_ma_lst = [15]
    period_ols_lst = [85, 95, 100, 105, 110, 120, 130, 140, 150, 90]
    K_lst = [1.4, 1.5, 1.7, 1.2, 1.6, 1.3, 1.1]
    ...
    for period_ols in period_ols_lst:
        ...
        for period_std in period_std_lst:
            df_ = df_signal.assign(m=lambda df: talib.MA(df.e_value.values, period_std))\
                ...
                .assign(e_v_n=lambda df: (df.e_value - df.m)/df.d)[['tickid', 'e_v_n', 'close_b', 'close_e',
                                                                    'close_b_1', 'close_e_1']]\
                .sort_values(['tickid'])
            for period_ma in period_ma_lst:
                group_ = group_day.assign(c_ma_ethbtc=lambda df: df.close - talib.MA(df.close.values, period_ma))\
