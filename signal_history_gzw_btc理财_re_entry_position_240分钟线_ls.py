...
if __name__ == '__main__':
    ...
    N5_lst = [4]  # 统计周期
    N6_lst = [3]  # 计数上限
    P_ATR_position_lst = [20]
    ATR_n_position_lst = [0.2]
    cut_position_lst = [1]
    status_days_lst = [37]
    P_ATR_LST = [i for i in range(33, 34, 3)]  # ATR周期
    ATR_n_lst = [i/10 for i in range(8, 9)]  # ATR倍数
                        ...
                        for N3 in N3_lst:
                            for N6 in N6_lst:
                                ...
                                for (s_date, e_date) in date_lst:
                                    ...
                                    # group_.to_csv('cl_trend/data/gzw_signal_' + symble + '_' + period + '.csv')
                                    if len(group_) < 20:
                                        continue
                                                    ...
                                                    for status_day in status_days_lst:
                                                        for cut_position in cut_position_lst:
                                                            ...
                                                            if (symble == 'ethindex') | (symble == 'eosbtc'):
                                                                ...
                                                                net_df.to_csv(
                                                                    'cl_trend/data/data/future_009_' + period + '_ethusd.csv')
                                                                ...
                                                                # signal_state.to_csv('cl_trend/data/signal_gzw_' + str(n) + '_' + symble + '.csv')
                                                                ...
                                                                net_df.to_csv(
                                                                    'cl_trend/data/data/future_009_' + period + '_xbtusd.csv')
                                                                # net_df[['date_time', 'net', 'close']].plot(
                                                                #     x='date_time', kind='line', grid=True,
                                                                #     title=symble + '_' + period)
                                                                #
                                                                ...
                                                                signal_state = pd.DataFrame(signal_lst,
                                                                                            columns=['s_time', 'e_time',
                                                                                                     ...
                                                                                                     'max_ret', 'hold_day',
                                                                                                     'position', 'bspk'])
                                                                # signal_state.to_csv('cl_trend/data/signal_gzw_' + str(
                                                                #     n) + '_' + symble + '.csv')
                                                                # df_lst.append(signal_state)
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'N2', 'N3', 'N1s', 'N2s', 'N3s', 'cut_position', 'P_ATR_position',
                                                    'ATR_n_position', 'status_day', 's_time', 'e_time'])
    print(signal_state)
    # signal_state.to_csv('cl_trend/data/state_gzw_usdt_re_entry_position' + str(n) + 'ls.csv')
