...
def maxRetrace(lst, n):
    ...
    new_lst = [new_lst[i] for i in range(0, len(new_lst), int(1440 / n))]
...
def yearsharpRatio(netlist, n):
    ...
    return np.mean(row) / np.std(row) * math.pow(365, 0.5)
...
def sharp_maxretrace_ann(netlist):
    ...
    for i in range(1, len(new_lst)):
        ...
        if 1 - new_lst[i] / max(new_lst[:i + 1]) > Max:
            Max = 1 - new_lst[i] / max(new_lst[:i + 1])
    return np.mean(row) / np.std(row) * math.pow(365, 0.5), Max, math.pow(netlist[-1] / netlist[0], 365 * 1440 / len(netlist)) - 1
