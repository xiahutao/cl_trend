...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(10, 13, 1)]  # 统计周期
    k_lst = [i / 100 for i in range(50, 51, 5)]  # 阈值
    p_lst = [i / 100 for i in range(40, 41, 5)]  # 阈值
    win_stop_lst = [10]
    P_ATR_LST = [36, 40, 32, 28, 24, 20, 16, 12]  # ATR周期
    ATR_n_lst = [1000000, 2.0, 1.9, 1.8, 1.7, 1.6, 1.5, 1.4, 1.3, 1.2, 1.1, 1]  # ATR倍数
    status_days_lst = [0]
    ...
    for symble in symble_lst:
        ...
        for N_ATR in P_ATR_LST:
            ...
            for ma in ma_lst:
                group__ = day_atr.assign(sign_l_1=lambda df: talib.MA(df['stat_sign_l'].shift(1).values, ma))\
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'k', 'p', 'win_stop', 'status_day', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_aapv_eth_' + str(n) + 'ls.csv')
