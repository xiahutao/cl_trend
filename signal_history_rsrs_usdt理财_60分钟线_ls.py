...
if __name__ == '__main__':
    ...
    n = 60  # 回测周期
    period = '60m'
    period_k0_lst = [50]  # N1 计算线性回归的K值
    period_std0_lst = [210]  # N2 计算斜率标准化的周期
    period_k1_lst = [30]  # N1 计算线性回归的K值
    period_std1_lst = [150]  # N2 计算斜率标准化的周期
    N_l_lst = [0.8]  # 多头开仓阈值
    N_s_lst = [-0.7]  # 空头开仓阈值
    win_stop_lst = [0.05, 0.1, 0.15, 0.25, 0.3, 0.2, 10]
                    ...
                    for period_k1 in period_k1_lst:
                        ...
                        for period_std1 in period_std1_lst:
                            ...
                            for (s_date, e_date) in date_lst:
                                ...
                                for period_std0 in period_std0_lst:
                                    if len(day_atr) <= period_std0:
                                        continue
                                    for period_std1 in period_std1_lst:
                                        if len(day_atr) <= period_std1:
                                            continue
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'period_k1', 'period_std1', 'N_l', 'N_s', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
