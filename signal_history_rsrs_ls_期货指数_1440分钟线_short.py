...
if __name__ == '__main__':
    ...
    period_k_lst = [40]  # N1 计算线性回归的K值
    period_std_lst = [250]  # N2 计算斜率标准化的周期
    N_l_lst = [0.8]  # 多头开仓阈值
    N_s_lst = [-0.7]  # 空头开仓阈值
    method_lst = [2]

    P_ATR_LST = [14, 16, 20, 12, 10, 22, 24, 26, 28, 30]  # ATR周期
    ATR_n_lst = [0.5, 0.6, 0.7, 0.4, 0.8, 0.9, 1.0]  # ATR倍数
                            ...
                            for N_l in N_l_lst:
                                for N_s in N_s_lst:
                                    ...
                                    for (s_date, e_date) in date_lst:
                                        group__ = group_[
                                            (group_['date_time'] >= s_date) & (group_['date_time'] <= e_date)]\
                                            .reset_index(drop=True)
                                        signal_lst = []
                                        trad_times = 0
                                        net = 1
                                        net_lst = []
                                        back_stime = group__.at[0, 'date_time']
                                        back_etime = group__.at[len(group__) - 1, 'date_time']
                                        ...
                                        if (symble == 'ethusdt') | (symble == 'eosbtc'):
                                            ...
                                            for idx, _row in group__.iterrows():
                                                if pos == 0:
                                            ...
                                            signal_state.to_csv('cl_trend/data/gzw_' + str(n) + '_' + symble + '_' + back_stime + 'ls.csv')
                                            # df_lst.append(signal_state)
                                            win_r, odds, ave_r, mid_r = get_winR_odds(signal_state.ret.tolist())
                                            win_R_3, win_R_5, ave_max = get_winR_max(signal_state.max_ret.tolist())
                                            state_row = []
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_rsrs_' + str(n) + 'short.csv')
