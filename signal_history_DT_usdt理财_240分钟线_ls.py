...
if __name__ == '__main__':
    ...
    road_period_l_lst = [i for i in range(3, 7)]  # 通道周期
    road_period_s_lst = [i for i in range(5, 8)]
    K1_lst = [i/100 for i in range(50, 80, 5)]  # 浮动倍数
    K2_lst = [i/100 for i in range(50, 80, 5)]  # 浮动倍数
    N1_lst = [10, 15]  # 均线周期
    win_stop_lst = [0.2, 0.15, 0.25, 0.3, 10]
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'road_period_l', 'K1', 'K2', 'N1', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_dt_usdt' + str(n) + 'ls.csv')
