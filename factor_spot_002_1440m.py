...
if __name__ == '__main__':
    ...
    period = '1440m'
    n = 1440
    min_n_lst = [i for i in range(6, 7)]
    loss_stop_lst = [1]
    win_stop_lst = [100000]
    fee = 0.002
    ...
    factor_lst = ["Alpha.alpha018", "Alpha.alpha050", "Alpha.alpha052", "Alpha.alpha055",
                  "Alpha.alpha060", "Alpha.alpha069", "Alpha.alpha071"]  # 正向因子001
    results = []
    ...
    for symbol in name_lst:
        ...
        c_ma = data[['tickid', 'close']]\
            .assign(c_ma=lambda df: df['close'] - tb.MA(df['close'].values, 20)).assign(symbol=symbol)[
            ['tickid', 'symbol', 'c_ma']]
    ...
    for min_n in min_n_lst:
        ...
        signal_df.to_csv('cl_trend/data/signal_df002.csv')
    ...
    res.to_csv('cl_trend/data/alpha002_' + str(n) + '.csv')
