...
#    print(os.path)
    ...
    e_date = '2018-06-30'
    n = 1440
    group = pd.read_csv('ethbtc_1440min_20170801_20180711.csv').reset_index()\
        .rename(columns={'minute': 'date_time'})
    print(group)
                ...
                for N3 in N3_lst:
                    for N_ATR in P_ATR_LST:
                        for N4 in N4_lst:
                            ...
                            print(N_ATR)
                            method = '4MA_' + str(N1) + '_' +str(N2)+ '_' + str(N3)+'_'+str(N4)+'_'+str(N_ATR) + '_' + str(ATR_n)
                            signal_lst = []
                            trad_times = 0
                            ...
                            if len(group) > N1:
                                net = 1
                                net_lst = []
                                group=group.assign(MA1=lambda df:tb.MA(df.close.values,N1)) \
                                            .assign(MA2=lambda df:tb.MA(df.close.values,N2)) \
                                            .assign(MA3=lambda df:tb.MA(df.close.values,N3)) \
                                            .assign(MA4=lambda df:tb.MA(df.close.values,N4))\
                                            .assign(atr=lambda df:tb.ATR(df.high.values,df.low.values,df.close.values,N_ATR))
    
                                group = group.assign(close_1=lambda df: df.close.shift(1)) \
                                            .assign(ma1_1=lambda df: df.MA1.shift(1)) \
                                            .assign(ma2_1=lambda df: df.MA2.shift(1)) \
                                            .assign(ma3_1=lambda df: df.MA3.shift(1)) \
                                            .assign(ma4_1=lambda df: df.MA4.shift(1)) \
                                            .assign(atr=lambda df: df.atr.shift(1)) \
                                            .assign(ma1_ma2=lambda df: df.ma1_1 - df.ma2_1) \
                                            .assign(c_ma=lambda df: df.close_1 - df.ma1_1) \
                                            .assign(ma3_ma4=lambda df: df.ma3_1 - df.ma4_1) \
                                            .assign(ma3_ma4_1=lambda df: df.ma3_ma4.shift(1))
                                position = 0
                                ...
                                for idx, _row in group.iterrows():
                                    ...
                                    if (position == 0) & (_row.ma3_ma4_1 <= 0) & (_row.c_ma > 0) & (_row.ma1_ma2 > 0) & (_row.ma3_ma4 > 0):
                                        position = 1
