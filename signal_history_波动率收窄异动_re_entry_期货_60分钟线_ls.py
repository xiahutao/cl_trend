...
if __name__ == '__main__':
    ...
    N1_lst = [8]  # 高低价周期
    N2_lst = [15]  # 唐奇安通道
    N3_lst = [i/100 for i in range(9, 10, 5)]  # 波动参数
            ...
            for N1 in N1_lst:
                for N2 in N2_lst:
                    ...
                    for (s_date, e_date) in date_lst:
                        ...
                        print(group_)
                        # group_.to_csv('cl_trend/data/lvt_signal_' + symble + '_' + period + '.csv')
                                ...
                                for win_stop in win_stop_lst:
                                    for status_day in status_days_lst:
                                        ...
                                        if (symble == 'ethindex') | (symble == 'eosbtc') | (symble == 'ethbtc'):
                                            ...
                                            net_df.to_csv(
                                                'cl_trend/data/data/future_015_' + period + '_ethusd.csv')
                                            ...
                                            # signal_state.to_csv('cl_trend/data/signal_lvt_' + str(n) + '_' + symble + '.csv')
                                            ...
                                            net_df.to_csv(
                                                'cl_trend/data/data/future_015_' + period + '_xbtusd.csv')
                                            ann_ROR = annROR(net_lst, n)
                                            total_ret = net_lst[-1]
                                            max_retrace = maxRetrace(net_lst, n)
                                            sharp = yearsharpRatio(net_lst, n)
                                            ...
                                            signal_state = pd.DataFrame(
                                                signal_lst, columns=['s_time', 'e_time',
                                                                     ...
                                                                     'bspk'])
                                            # signal_state.to_csv(
                                            #     'cl_trend/data/signal_lvt_' + str(n) + '_' + symble + '.csv')
                                            # df_lst.append(signal_state)
