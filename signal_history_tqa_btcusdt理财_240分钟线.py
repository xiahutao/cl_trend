...
if __name__ == '__main__':
    ...
    # symble = 'yeeeth'
    x = 8
    symble_lst = ['btcindex', 'xbtusd', 'btcusdt' ]
    n = 240  # 回测周期
    period = '240m'

    road_period_l_lst = [i for i in range(8, 10, 1)]  # 唐奇安通道周期
    road_period_s_lst = [i for i in range(6, 8, 1)]  # 唐奇安通道周期
    ma_period_lst = [15]
    P_ATR_LST = [14, 16, 20, 12, 18, 22, 24, 26, 28, 30]  # ATR周期
    ATR_n_lst = [0.5, 0.6, 0.7, 0.4, 0.8, 0.9]  # ATR倍数
    win_stop_lst = [0.15]
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'road_period_s', 'ma_period', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_tqa_usdt_' + str(n) + 'ls.csv')
