...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(13, 14)]  # 高低价周期
    N2_lst = [i for i in range(18, 19, 2)]  # 唐奇安通道
    N3_lst = [i / 100 for i in range(9, 10, 3)]  # 波动参数
    P_ATR_LST = [36]  # ATR周期
    ATR_n_lst = [1.1]  # ATR倍数
    win_stop_lst = [0.25]
    ...
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr']] \
            ...
            .reset_index(drop=True)[['tickid', 'atr', 'close']]\
        ...
        for N1 in N1_lst:
            for N2 in N2_lst:
                ...
                group_ = day_atr \
                    ...
                    ['tickid', 'H_n_1', 'L_n_1', 'M_n_1', 'atr', 'para_1']] \
                        ...
                        for win_stop in win_stop_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                if position == 0:
                                    ...
                                    if (_row.close > _row.H_n_1) & (_row.para_1 < N3):
                                        cost = _row.close
                                        position = 1
                                    ...
                                    elif (_row.close < _row.L_n_1) & (_row.para_1 < N3):
                                        cost = _row.close
                                        position = -1
                                        s_time = _row.date_time
                                        value_start = value
                                    ...
                                    if (_row.close < _row.L_n_1) & (_row.para_1 < N3):
                                        ...
                                        hold_price.append(cost)
                                        low_price.append(cost)
                                    elif _row.close < max(max(hold_price) - _row.atr * ATR_n, _row.M_n_1):
                                        e_time = _row.date_time
                                        if cont_btc > 0:
                                            p_price_btc = _row.close_b * (1 - fee)
                                            p_price_eth = _row.close_e * (1 + fee)
                                    ...
                                    elif _row.close > min(min(hold_price) + _row.atr * ATR_n, _row.M_n_1):
                                        e_time = _row.date_time
                                        if cont_btc > 0:
                                            p_price_btc = _row.close_b * (1 - fee)
                                            p_price_eth = _row.close_e * (1 + fee)
                            ...
                            signal_state_all = pd.DataFrame(
                                signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                     ...
                                                     'position'])
                            signal_state_all.to_csv('cl_trend/data/signal_315_' + str(n) + '.csv')
                            for (s_date, e_date) in date_lst:
                                ...
                                # net_df.to_csv('cl_trend/data/data/315_' + period + '.csv')
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'win_stop', 'back_stime', 'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_lvt_' + str(n) + '.csv')
