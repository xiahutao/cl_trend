...
if __name__ == '__main__':
    ...
    symble_lst = ['ethbtc', 'eosbtc', 'etcbtc', 'iotabtc', 'iostbtc', 'ltcbtc', 'neobtc', 'trxbtc', 'xrpbtc',
                 'zrxbtc', 'omgbtc', 'xlmbtc']
    # symble_lst = ['xlmbtc']
    n = 60  # 回测周期
    period = '60m'

    N1_lst = [i for i in range(10, 101, 10)]  # 短周期周期1
    N2_lst = [i for i in range(10, 101, 10)]  # 长周期周期2

    K1_lst = [i / 100 for i in range(10, 96, 20)]  # 近期比
    K2_lst = [i / 100 for i in range(10, 96, 20)]  # 长期比例
    ...
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_ocm_reverse_' + str(n) + '_now.csv')
