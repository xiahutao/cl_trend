...
if __name__ == '__main__':
    ...
    ma_lst = [i for i in range(9, 10)]  # 统计周期
    k_lst = [i / 100 for i in range(45, 46, 5)]  # 阈值
    p_lst = [i / 100 for i in range(40, 41, 5)]  # 阈值
    ...
    del df_btc
    del df_eth
    hq_data = pd.read_csv('data/ethxbt_1m.csv').loc[
              :, ['tickid', 'close', 'high', 'low', 'open']]
    hq_data = hq_data[hq_data['tickid'] >= 1493568000]
    group = pd.read_csv('data/ethxbt_' + period + '.csv').loc[:, ['tickid', 'open', 'high', 'low', 'close']]
    group = group[group['tickid'] >= 1493568000].sort_values(['tickid'])\
        .assign(close_1=lambda df: df.close.shift(1))
    group_day = pd.read_csv('data/ethxbt_' + '1440m' + '.csv').loc[:, ['tickid', 'open', 'high', 'low', 'close']]
    ...
    group = group.assign(pivot1=lambda df: (df.open + df.low + df.close) / 3) \
        .assign(pivot2=lambda df: (df.high + df.open + df.close) / 3) \
        .assign(resis_l=lambda df: (2 * df.pivot1 - df.low - df.close)) \
        .assign(suport_s=lambda df: (df.close - (2 * df.pivot2 - df.high))) \
        .assign(stat_sign_l=lambda df: df.resis_l.apply(lambda x: transfer_resis_stat(x))) \
        .assign(stat_sign_s=lambda df: df.suport_s.apply(lambda x: transfer_resis_stat(x)))
    state_lst = []
    for N_ATR in P_ATR_LST:
        ...
        day_atr = group_day[['tickid', 'atr']] \
            ...
            .reset_index(drop=True)[['tickid', 'atr', 'stat_sign_l', 'stat_sign_s']]
        print(day_atr)
        for ma in ma_lst:
            group_ = day_atr.assign(sign_l_1=lambda df: talib.MA(df['stat_sign_l'].shift(1).values, ma))\
                .assign(sign_s_1=lambda df: talib.MA(df['stat_sign_s'].shift(1).values, ma))[
                ['tickid', 'sign_l_1', 'sign_s_1', 'atr']] \
                ...
                for p in p_lst:
                    ...
                    for ATR_n in ATR_n_lst:
                        ...
                        for idx, _row in group_.iterrows():
                            if position == 0:
                                if (_row.sign_l_1 > k) & (_row.date_time[:x] != e_time[:x]):
                                    cost = _row.open
                                    position = 1
                                ...
                                elif (_row.sign_s_1 > k) & (_row.date_time[:x] != e_time[:x]):
                                    cost = _row.open
                                    position = -1
                                    s_time = _row.date_time
                                    value_start = value
                                ...
                                if (_row.sign_s_1 > k) & (_row.date_time[:x] != s_time[:x]):
                                    ...
                                    hold_price.append(cost)
                                    low_price.append(cost)
                                elif (_row.sign_s_1 > p) & (_row.date_time[:x] != s_time[:x]):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                    ...
                                    signal_lst.append(signal_row)
                                    position = 0
                                elif (_row.sign_l_1 <= k) & (_row.close < max(hold_price) - _row.atr * ATR_n) & (_row.date_time[:x] != s_time[:x]):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                ...
                                if (_row.sign_l_1 > k) & (_row.date_time[:x] != s_time[:x]):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                    ...
                                    hold_price.append(cost)
                                    high_price.append(cost)
                                elif (_row.sign_l_1 > p) & (_row.sign_s_1 <= k) & (_row.date_time[:x] != s_time[:x]):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                                ...
                                elif ((_row.sign_s_1 <= k)) & (_row.close > min(hold_price) + _row.atr * ATR_n) & (_row.date_time[:x] != s_time[:x]):
                                    e_time = _row.date_time
                                    if cont_btc > 0:
                                        p_price_btc = _row.close_b * (1 - fee)
                                        p_price_eth = _row.close_e * (1 + fee)
                        ...
                        signal_state_all = pd.DataFrame(
                            signal_lst, columns=['s_time', 'e_time', 'cont_btc', 'cont_eth', 'k_price_btc',
                                                 ...
                                                 'position'])
                        signal_state_all.to_csv('cl_trend/data/signal_316_' + str(n) + '.csv')
                        for (s_date, e_date) in date_lst:
                            ...
                            # net_df.to_csv('cl_trend/data/data/316_' + period + '.csv')
    ...
    res = pd.DataFrame(state_lst,
                       columns=['period', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp', 'max_retrace',
                                ...
                                'win_r_3_p', 'win_r_5_p', 'ave_max_p', 'art_N', 'art_n', 'ma', 'k', 'p', 'back_stime',
                                'back_etime'])
    res.to_csv('cl_trend/data/neu_strategy_aapv_' + str(n) + '.csv')
