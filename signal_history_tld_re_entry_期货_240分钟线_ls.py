...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(7, 8, 1)]  # 短周期
    N2_lst = [i for i in range(50, 51, 5)]  # 中周期
    N3_lst = [i for i in range(85, 86, 5)]  # 长周期
    N4_lst = [i for i in range(22, 23, 1)]  # 唐奇安通道数
    P_ATR_LST = [34, 38, 42, 30, 26, 22, 18, 14, 10]  # ATR周期
    ATR_n_lst = [1000000, 1.5, 1.4, 1.3, 1.2, 1.1, 1.0, 0.9, 0.8, 0.7, 0.6, 0.5]  # ATR倍数
    status_days_lst = [0]
    win_stop_lst = [10]
    ...
    fee = 0.002
    date_lst = [('2017-01-01', '2018-01-01'), ('2018-01-01', '2018-07-01'), ('2018-07-01', '2019-02-30')]
                ...
                for N2 in N2_lst:
                    for N3 in N3_lst:
                        if (N1 < N2) & (N2 < N3) & (len(group) > N3):
                            group['ma1'] = talib.MA(group['close_1'].values, N1)
                            group['ma2'] = talib.MA(group['close_1'].values, N2)
                            group['ma3'] = talib.MA(group['close_1'].values, N3)
                            for N4 in N4_lst:
                                group = group.assign(hhv=lambda df: talib.MAX(df['high'].shift(1).values, N4))\
                                        ...
                                        for win_stop in win_stop_lst:
                                            ...
                                            if (symble == 'ethusd') | (symble == 'eosbtc') | (symble == 'ethbtc'):
                                                ...
                                                for idx, _row in group_.iterrows():
                                                    ...
                                                    if pos == 0:
                                                        if (_row.ma1 > _row.ma2) & (_row.close_1 > _row.ma3) & \
                                                                (_row.high > _row.hhv):
                                                            cost = _row.hhv * (1 + fee)
                                                            if _row.open > _row.hhv:
                                                        ...
                                                        elif (_row.ma1 < _row.ma2) & (_row.close_1 < _row.ma3) & \
                                                                (_row.low < _row.llv):
                                                            cost = _row.llv * (1 - fee)

                                                            if _row.open < _row.llv:
                                                                cost = _row.open * (1 - fee)
                                                            pos = -1
                                                            s_time = _row.date_time
                                                            hold_price = []
                                                        ...
                                                        if pos == 1:
                                                            if _row.low < _row.llv:
                                                                s_price = _row.llv * (1 - fee)
                                                                if _row.open < _row.llv:
                                                            ...
                                                            if _row.high > _row.hhv:
                                                                b_price = _row.hhv * (1 + fee)
                                                                if _row.open > _row.hhv:
                                                ...
                                                # net_df.to_csv('data/tqa_' + symble + '_' + 'net_.csv')
                                                ...
                                                # signal_state.to_csv('cl_trend/data/signal_tqa_' + str(
                                                #     status_day) + '_' + symble + '_' + back_stime + 'ls.csv')
                                                ...
                                                #         0])
                                                # net_df.to_csv(
                                                #     'data/btc_tqa_btcindex_' + period + '.csv')
                                                # net_df[['date_time', 'net', 'close']].plot(
                                                #     x='date_time', kind='line', grid=True,
                                                #     title=symble + '_' + period)
                                                #
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'N2', 'N3', 'N4', 'win_stop', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv('cl_trend/data/state_tld_entry' + str(n) + 'ls.csv')
